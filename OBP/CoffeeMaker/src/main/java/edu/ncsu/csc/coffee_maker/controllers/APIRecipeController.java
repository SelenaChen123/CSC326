...
public class APIRecipeController extends APIController {
    ...
    public ResponseEntity createRecipe ( @RequestBody final Recipe recipe ) {
        final Set<Ingredient> s = new HashSet<Ingredient>( recipe.getIngredients() );
        if ( s.size() != recipe.getIngredients().size() ) {
        ...
        if ( Recipe.getAll().size() < 3 && Recipe.isValidRecipe( recipe ) ) {
            recipe.save();
     ...
     * to edit a Recipe. Invalid JSON will fail.
     *
     * @param recipe
    ...
    public ResponseEntity editRecipe ( @PathVariable ( "name" ) final String name ) {
        final Recipe recipe = Recipe.getByName( name );
        if ( null == Recipe.getByName( recipe.getName() ) ) {
            return new ResponseEntity( successResponse( "Recipe with the name " + recipe.getName() + " doesn't exist" ),
                    HttpStatus.CONFLICT );
        }
        if ( Recipe.getAll().size() < 3 && Recipe.isValidRecipe( recipe ) ) {
            recipe.save();
