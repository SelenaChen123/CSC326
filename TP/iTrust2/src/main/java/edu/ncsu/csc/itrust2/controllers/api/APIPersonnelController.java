...
public class APIPersonnelController extends APIController {
    ...
    @PreAuthorize ( "hasAnyRole('ROLE_HCP', 'ROLE_OD', 'ROLE_OPH', 'ROLE_LABTECH', 'ROLE_ER', 'ROLE_ADMIN', 'ROLE_VIROLOGIST', 'ROLE_PHARMACIST')" )
    ...
    public ResponseEntity getPersonnelByRole ( @PathVariable ( "role" ) final String role ) {
        ...
        else if ( role.equals( Role.ROLE_PHARMACIST.toString() ) ) {
            return new ResponseEntity( User.getByRole( Role.ROLE_PHARMACIST ), HttpStatus.OK );
        }
